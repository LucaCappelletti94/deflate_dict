...
def _deflate(my_dict: Dict, sep="_", root="", leave_tuples:bool=False)->Generator:
    ...
    for key, value in my_dict.items():
        flatten_key = "{root}{separator}{key}".format(root=root, separator=separator, key=type_encode(key))
        if is_leaf(value) or leave_tuples and isinstance(value, tuple):
            yield (flatten_key, value)
        else:
            for sub_key, sub_value in _deflate(encode_list(value) if isinstance(value, list) else value, sep=sep, root=flatten_key):
